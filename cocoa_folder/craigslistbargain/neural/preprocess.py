...
from cocoa_folder.cocoa.core.util import read_pickle, write_pickle, read_json
from cocoa_folder.cocoa.core.entity import Entity, CanonicalEntity, is_entity
from cocoa_folder.cocoa.model.vocab import Vocabulary

from cocoa_folder.craigslistbargain.core.price_tracker import PriceTracker, PriceScaler
from cocoa_folder.craigslistbargain.core.tokenizer import tokenize
from cocoa_folder.craigslistbargain.neural.batcher import DialogueBatcherFactory, Batch
from cocoa_folder.craigslistbargain.neural.symbols import markers
from cocoa_folder.craigslistbargain.neural.vocab_builder import create_mappings
from cocoa_folder.craigslistbargain.neural import make_model_mappings
...
class TextIntMap(object):
    ...
    def int_to_text(self, inds, stage=None, prices=None):
        ...
        if prices is not None:
            ...
            toks = [CanonicalEntity(value=p, type='price') if price_filler(x) else x for x, p in zip(toks, prices)]#izip(toks, prices)]
        return toks
...
class Dialogue(object):
    ...
    def convert_to_int(self):
        ...
        for turn in self.token_turns:
            ...
            for portion, stage in zip(self.turns, ('encoding', 'decoding', 'target')):#izip(self.turns, ('encoding', 'decoding', 'target')):
...
class DataGenerator(object):
    ...
    def get_all_responses(self, name):
        ...
        for dialogue in dialogues:
            for turn, role in zip(dialogue.token_turns, dialogue.roles):#izip(dialogue.token_turns, dialogue.roles):
