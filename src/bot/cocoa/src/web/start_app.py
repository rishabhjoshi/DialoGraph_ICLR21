...
def add_website_arguments(parser):
    ...
    parser.add_argument('--strat_model', type=str,
                        choices=['hed', 'fst', 'rnn', 'transformer', 'graph'], default = 'graph')
...
def add_systems(config_dict, schema, lexicon, realizer, strat_model):
    """
    Params:
    ...
    for (sys_name, info) in config_dict.items():
        ...
        if info["active"]:  # TODO Verify this
            ...
            if type == SimpleSystem.name():
                model = SimpleSystem(lexicon, timed_session=True, realizer=realizer, consecutive_entity=False, strat_model = strat_model)
...
if __name__ == "__main__":
    ...
    if 'strat_model' not in params.keys():
        params['strat_model'] = args.strat_model

    systems, pairing_probabilities = add_systems(params['models'], schema, lexicon, realizer, params['strat_model'])
