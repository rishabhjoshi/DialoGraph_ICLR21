...
class Messages(object):
    ...
    YouLeftRoom = "Great, you've completed the chat!"#'You skipped the chat. '
...
class User(object):
    def __init__(self, row):
        ...
        self.chat_id = row[11]
    def print_user(self):
        print ('Name : {}, Status : {}, Status time stamp : {}, Connected status : {}, connected time stamp : {}, message : {}, partner type: {}, partner id : {}, scenario id :{}, agent index : {}, selected index : {}, chat_id: {}'.format(self.name, self.status, self.status_timestamp, self.connected_status, self.connected_timestamp, self.message, self.partner_type, self.partner_id, self.scenario_id, self.agent_index, self.selected_index, self.chat_id))
...
class BackendConnection(object):
    ...
    def _get_user_info(self, cursor, userid, assumed_status=None):
        ...
        u = self._get_user_info_unchecked(cursor, userid)
        import sys#u = User(self._ensure_not_none(x, NoSuchUserException))
        try:
            if type(u.agent_index) != int:
                u.agent_index = int.from_bytes(u.agent_index, byteorder=sys.byteorder)#u = User(self._ensure_not_none(x, NoSuchUserException))
        except:
            import pdb; pdb.set_trace()
        #print ('USER INFO : ', u.print_user())#u = User(self._ensure_not_none(x, NoSuchUserException))
    ...
    def attempt_join_chat(self, userid):
        ...
        def _pair_with_bot(cursor, userid, my_index, bot_type, scenario, chat_id):
            ...
            self.sessions[userid] = my_session
            #print ('MY INDEX : ', my_index)#self.sessions[userid] = my_session
    ...
    def receive(self, userid):
        ...
        if controller is None:
            ...
            return None
        controller.step(self)                               # THIS IS CALLED SECOND
            ...
            with self.conn:
                ...
                cursor.execute('INSERT INTO survey VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',
                               (userid, user_info.chat_id, user_info.partner_type,
                                ...
                                'NA',#data['n06_texts'],
                                'NA',#data['n07_tech'],
                                'NA',#data['n08_learn_spa'],
                                'NA',#data['n09_learn_eng'],
                                data['n10_age'],
                                data['n11_ability_spa'],
                                'NA',#data['n12_ability_eng'],
                                data['n13_country'],
                                'NA',#data['n14_online_spa'],
                                'NA',#data['n15_online_eng'],
                                'NA',#data['n16_online_mix'],
                                data['n17_comments']))
